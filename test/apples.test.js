describe("The applesFActory", function() {
	...
	it("should be able to calculate the total number of apples sold", function() {
		 ...
		 assert.equal(soldList,totalNumberSold(applesSoldList1))
	...
	it("should be able to calculate the total income from all the apples sold", function() {
		assert.equal(allIncome, totalIncome(applesSoldList1));
		assert.equal(allIncome, totalIncome(applesSoldList2));
	});

	it("should be able to calculate the total income from all the green apples sold", function() {
		assert.equal(allGreenQnty, totalIncomeGreenApples(applesSoldList1));
		assert.equal(allGreenQnty, totalIncomeGreenApples(applesSoldList2));
	});

	it("should be able to calculate the total profit from apples sold", function() {
		
		assert.equal(allProfit, totalProfit(applesSoldList1, applePrices1));
